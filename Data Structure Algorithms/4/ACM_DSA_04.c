...
void selection_sort(int length_arr, int num_arr[]) {
	...
	while (i < length_arr - 1){
		...
		for (j = i; j < length_arr	; j++) { if (num_arr[j] < num_arr[min_index]) { min_index = j; } }

		switch_vars(&num_arr[i], &num_arr[min_index]);
